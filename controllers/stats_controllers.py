...
def reload_stats_data():
    """
    Reloads data from S3 bucket into memory in the CacheData object.
    """
    try:
        data_obj.load_data()
...
SYMBOL_DICT = {1: "D", 7: "W", 30: "M", 90: "Q"}
AGG_LEVEL_DICT = {1: "daily", 7: "weekly", 30: "monthly", 90: "quarterly"}
...
def manipulate_data(metrics, date_range_min, date_range_max):
    ...
    for metric in metrics:
        temp_dur = data_obj.df.loc[
            (data_obj.df["Title"] == metric["Title"])
    ...
    for metric in metrics:
        ...
        if col_name not in secondary_y_dict.keys():
            ...
            temp = data_obj.df.loc[
                ...
                & (data_obj.df["Period"] < date_range_max)
            ].copy()
            calculation = (
                "sum"
                if pd.isnull(GROUP_BY_DICT.get(metric["Title"]))
                else GROUP_BY_DICT.get(metric["Title"])
            )
            print(metric["Title"], calculation)
            temp = temp.groupby("Period").agg(
                {"Value": calculation}
            )  # [['Value']].sum()
    ...
    return (
        result_df,
        f"(Aggregated at a {AGG_LEVEL_DICT[max_duration]} level)",
        secondary_y_dict,
    )
...
def get_titles_catagory():
    ...
    application.logger.info("/get_titles")
    return jsonify(
        {"data": data_obj.get_titles_catagory(cat, is_regional, is_industry)}
    )
...
def get_graphs():
    ...
    dff, agg_level, secondary_y_dict = manipulate_data(
        metrics, date_range_min, date_range_max
    )
    for col in dff.columns:
        chart1_fig.add_trace(
            go.Scatter(x=dff.index, y=dff[col], name=col),
            secondary_y=secondary_y_dict[col],
        )
    ...
    chart2_fig.update_layout(
        height=350, margin=dict(l=10, r=10, b=10), font=dict(size=9)
    )
    chart2_fig.update_xaxes(tickangle=45)  # visible=False
    return jsonify(
        {
            "chart1": chart1_fig.to_json(),
